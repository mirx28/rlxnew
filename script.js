...
function createSlide(side, index) {
  ...
  // Slide 5 (About us): the two founder portraits are re-cropped so both heads
  // sit at the same height; bias object-position up so the frame keeps the head
  // visible and aligned across the left/right columns on any screen.
  if (data.title === "About us") {
    const img = el.querySelector("img");
    if (img) img.style.objectPosition = "50% 24%";
  }
...
function animateSlider() {
  ...
  // Only repaint when the slider actually moved. This avoids constant
  // clip-path/transform writes that stutter on older/lower-end laptops.
  if (Math.abs(scrollPosition - lastRenderedSlider) > 0.0002) {
    lastRenderedSlider = scrollPosition;
    updateSlider();
  }
}
...
// RLX: keep pin distances in sync with the viewport, but ignore height-only
// resizes. Android/Samsung browsers resize the viewport (and fire "resize")
// every time the address bar hides/shows while the user is scrolling, unlike
// iOS Safari. Calling ScrollTrigger.refresh() on those height-only events
// recalculates the pinned .slider/.spotlight sections mid-scroll and traps
// the scroll. Only refresh on a real width change (rotation/actual resize),
// and debounce it.
(function () {
